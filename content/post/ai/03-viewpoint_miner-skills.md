...
这不是 LLM 不够聪明，而是**工具太多，正确的路径只有一条**。27 个工具摆在面前，LLM 需要自己做排列组合——而大多数时候，正确答案只有一个组合。与其让它猜，不如直接告诉它。
...
Skill 不是一个程序模块。它是一个 **Markdown 文件**，包含：

- **名字和描述**：LLM 判断"这个 Skill 适不适合当前场景"的唯一依据
...
trade-copier 项目里有 5 个 Skill，每个 Skill 对应一类用户意图，有自己的工具链和边界策略。
...
这不是束缚 LLM 的表达能力，而是保证输出**信息密度可控**。LLM 可以在这个框架内自由措辞，但不会漏掉关键信息（比如忘了提价格），也不会过度展开（比如把 50 条信号全部列出来）。
...
```
avg_actions_per_stock >= 2.5  →  偏短线（高频操作，持股周期短）
avg_actions_per_stock < 2.5   →  偏波段（节奏慢，持仓时间长）
```

如果这个判断逻辑不在 Skill 里，LLM 拿到 `avg_actions_per_stock: 3.2` 这个数字后，需要自己推理"3.2 算高还是低？"——它没有基准。Skill 提供了基准和解读逻辑，LLM 只需要代入数据。
...
关键是触发语要覆盖**意图类别**，而不是具体的措辞。五个 Skill 覆盖了五种意图：个股分析、作者分析、全局扫描、主题搜索、推送操作——基本涵盖了用户对这个系统可能有的所有问题。
...
因为 LLM 的主要困难不是"不知道有哪些工具"，而是"不知道哪个组合最优"。Skill 提供的是**经过验证的最优路径**。LLM 在这条路径上走，成功率基本 100%。深水区工具（如 `build_stock_context`）放在 "Deep dive (optional)" 里作为可选项——LLM 在需要更多细节时自然会选。

### 3. 边界处理写进 Skill，LLM 不需要临场发挥

每个 Skill 都有 "Edge Cases" 章节。这不是教科书式的"考虑周全"——它是让 LLM 在异常情况下有**确定的、可预期的行为**。
...
如果没有这些，LLM 在搜索无结果时可能会说"数据库可能有问题"（没用的猜测），或者"搜索不到相关内容"（太敷衍）。Skill 把每种异常映射到了一个**可操作的建议**。
...
这个分离的价值在于：SKILL.md 保持简洁，聚焦于**工作流和决策逻辑**；参数细节在 references 里，LLM 只有在需要确认参数格式时才去读。如果所有参数表都堆在 SKILL.md 里，五个 Skill 会让 LLM 的上下文窗口被大量工具文档占满——而大部分时候参数细节根本用不到。
...
区别在于：**LLM 的决策从"开放式选择题"变成了"填空题"**。它不再需要猜测应该用什么工具——Skill 已经选好了。它只需要填参数（股票名）和合成输出。
...
这引出一个重要的架构选择：**MCP 工具应该保持细粒度、高内聚**，而**Skill 负责把它们编排成工作流**。不要为了"减少 LLM 选择困难"就把多个工具合并成一个——那是用复合工具代替 Skill，牺牲了灵活性。正确的做法是：工具保持原子性，Skill 提供组合方式。
